...
# weights
weights = [[1.0, 1.0], [1.0, 1.3], [1.0, 1.5], [1.0, 2.0]]

#################### File Path ####################
current_datetime = datetime.now().strftime("%y%m%d_%H%M%S")
print("Current Date and Time: ", current_datetime)

data_dir = f'/data/ananya012/results/Gender/PostFinalReport/TuningRuns/Tuning_weights_' + str(current_datetime)
...
with open(data_dir + '/parameters.txt', 'w') as f:
    ...
    f.write(f"GCN Hidden Channels: {hidden_channels}\n")
    f.write(f"HC2: {hc2}\n")
    f.write(f"HC4: {hc4}\n")
...
def train(model, optimizer, train_loader, train_label, device, weights):
    model.train()
...
def test(model, test_loader, test_label, device, weights):
    model.eval()
    predictions = []
    targets = []
    overall_attention = []
...
for ws in weights:
    #################### File Definitions ####################
    cur_params = f'/weights_{ws[0]}_{ws[1]}'
    ...
    for fold in range(1, 6):
        ...
        for epoch in range(epoch_num):
            ...
            train(model, optimizer, train_loader, train_label_tensor, device, ws)
            train_loss, train_accuracy, train_predictions, train_targets, train_attention = test(model, train_loader, train_label_tensor, device, ws)
            test_loss, test_accuracy, test_predictions, test_targets, test_attention = test(model, test_loader, test_label_tensor, device, ws)
